...
class Timeline:
    ...
    def compute_statistics(self) -> TimelineStatistics:
        ...
        if self._by_month:
            busiest_month, month_ids = max(
                self._by_month.items(),
                key=lambda x: len(x[1])
            )
        ...
        # Platform breakdown
        platform_breakdown = [
            self.get_platform_activity(platform)
            for platform in self._by_platform.keys()
        ]
    ...
    def sample_memories(self, n: int, strategy: str = "stratified") -> list[Memory]:
        ...
        if strategy == "random":
            ...
            samples_per_year = {
                year: max(1, int(n * len(mems) / len(dated)))
