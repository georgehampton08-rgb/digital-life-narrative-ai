...
class PrivacySettings(BaseModel):
    ...
    enabled_categories: set[DataCategory] = Field(
    ...
    def to_user_summary(self) -> str:
        ...
        lines.append(f"AI Analysis: {'Enabled' if self.is_ai_enabled() else 'Disabled (Local Only)'}")
...
class PrivacyGate:
    ...
    **CRITICAL**: Never bypass this class. Direct AI calls = security bug.
    ...
    def __init__(self, settings: PrivacySettings | None = None):
        ...
        logger.info(f"PrivacyGate initialized: session={self.session_id}, level={self.settings.privacy_level.value}")
    ...
    def grant_consent(
        self, user_acknowledgment: str = "User granted consent"
    ) -> ConsentReceipt:
    ...
    def revoke_consent(self) -> None:
        ...
        if self.current_consent:
            logger.warning(
                f"Consent revoked: receipt_id={self.current_consent.id}"
            )
        ...
        **THE CRITICAL METHOD**
        ...
        for memory in memories:
            try:
                ...
                warnings.append(
                    f"Failed to prepare memory {memory.id}: {str(e)}"
                )
        ...
        if not is_safe:
            raise PrivacyViolationError(
                f"Privacy violations detected: {', '.join(violations)}"
            )
        ...
        if consent and consent.covers_category(DataCategory.PEOPLE):
            if level == PrivacyLevel.MINIMAL:
                ...
                if self.settings.anonymize_people:
                    output["people"] = [
                        self.anonymize_name(p.name) for p in memory.people
                    ]
                else:
    ...
    def record_transmission(
        self, payload: list[dict], destination: str, response_received: bool = False,
        error: str | None = None
    ) -> TransmissionRecord:
                ...
                if isinstance(value, str):
                    ...
                    if "/" in value or "\\" in value:
                        if key != "id":  # IDs can have / in UUIDs
                            violations.append(
                                f"Potential file path in {key}: {value[:50]}"
                            )
                    ...
                    if pii_types:
                        violations.append(
                            f"PII detected in {key}: {', '.join(pii_types)}"
                        )
            ...
            if "coordinates" in item or "approximate_coordinates" in item:
                ...
                if coords and isinstance(coords, dict):
                    ...
                    if "." in lat_str and len(lat_str.split(".")[1]) > 4:
                        violations.append(
                            f"Coordinates too precise: {lat}, {lon}"
                        )
    ...
    def wrapper(*args, **kwargs):
        ...
        if not is_valid:
            raise ConsentRequiredError(
                f"AI access denied: {reason}. "
                f"Call gate.request_consent() first."
            )
