...
class AnalysisProgress:
    ...
    def to_status_line(self) -> str:
        ...
        if self.current_chapter:
            return (
                f"[{pct:3.0f}%] {self.stage}: {self.current_chapter} - {self.message} ({elapsed})"
            )
...
class LifeStoryAnalyzer:
    ...
    def analyze(
                ...
                if cached_payload is not None:
                    ...
                    try:
                        ...
                        self._logger.warning(
                            f"Cache reconstruction failed: {e}. Proceeding with full analysis."
                        )
                else:
        ...
        try:
            ...
            if (
                ...
                and media_fp is not None
                and config_fp is not None
            ):
                try:
                    meta = CacheMeta(
    ...
    def _analyze_gaps(
        ...
        for g in significant_gaps:
            gaps_data_lines.append(f"- {g.start_date} to {g.end_date}: {g.duration_days} days")
        ...
        for g in significant_gaps[:5]:
            ...
            before = (
                timeline.get_memories_in_range(
    ...
    def _generate_executive_summary(
        ...
        chapters_summary = prepare_chapters_for_prompt([c.to_dict() for c in chapters])
    ...
    def _sample_memories_for_detection(
        ...
        if by_year:
            ...
            for year in sorted(by_year.keys()):
                ...
                year_memories.sort(
                    key=lambda m: (
                        (1 if m.location else 0) + (1 if m.people else 0) + (1 if m.caption else 0)
                    ),
                    reverse=True,
                )
    ...
    def _assign_memories_to_chapters(
        ...
        for memory in memories:
            ...
            memory_date = (
                memory.timestamp.date() if hasattr(memory.timestamp, "date") else memory.timestamp
            )
    ...
    def _assess_data_quality(self, stats: TimelineStatistics) -> list[str]:
        ...
        if stats.undated_memories > 0:
            pct = (
                (stats.undated_memories / stats.total_memories) * 100 if stats.total_memories else 0
            )
            notes.append(f"{stats.undated_memories} memories ({pct:.1f}%) lack timestamps")

        if stats.location_coverage < 0.2:
            notes.append(
                f"Only {stats.location_coverage * 100:.0f}% of memories have location data"
            )
