...
from src.ai.sampling import SamplingStrategy, BudgetManager, plan_visual_sampling
from src.ai.visual_tagger import VisualTagger, VisualTagResult, ChapterVisualSummary
from src.ai.cache import AICache, fingerprint_analysis_config
...
class LifeStoryAnalyzer:
    ...
    def _detect_chapters(self, timeline: Timeline, config: AnalysisConfig) -> list[LifeChapter]:
        ...
        safe_payloads = [
            m.to_ai_payload(privacy_level=self.config.privacy.level.value) 
            for m in memories
        ]
        ...
        for idx, chapter_dict in enumerate(chapters_data):
            try:
                ...
                chapter.media_count = len(chapter_mems)
    ...
    def _sample_memories_for_detection(
        ...
        from src.core.models import DepthMode
        ...
        depth_cfg = self.client._config.ai.get_depth_config()
    ...
    def _generate_executive_summary(
        ...
        from src.ai.prompts import get_prompt, format_timeline_visual_progression, prepare_platform_breakdown, prepare_chapters_for_prompt
