...
from dlnai.core.memory import ConfidenceLevel, SourcePlatform
...
class DetectionResult:
    ...
    def to_summary(self) -> str:
        ...
        emoji = (
            ...
            else "?" if self.confidence == ConfidenceLevel.LOW else "~"
        )
...
def scan_directory_structure(
    path: Path, max_depth: int = 3, sample_size: int = 100
) -> DirectorySummary:
...
def detect_snapchat(summary: DirectorySummary, root: Path) -> Optional[DetectionResult]:
    ...
    if has_memories_history:
        ...
        has_memories_dir
        or has_snap_history
        or any("_memories" in f for f in summary.top_level_files)
    ):
...
def detect_google_photos(summary: DirectorySummary, root: Path) -> Optional[DetectionResult]:
    ...
    if summary.json_file_count > 0 and summary.media_file_count > 0:
        ...
        if 0.5 <= ratio <= 1.5:  # Similar counts
            ...
            evidence.append(
                f"Found JSON sidecar pattern ({summary.json_file_count} JSON, {summary.media_file_count} media files)"
            )
