...
Example:
    >>> from dlnai.core.models import Timeline, Memory
    >>>
...
class TimelineStatistics(BaseModel):
    ...
    earliest_memory: datetime | None = None
    latest_memory: datetime | None = None
    ...
    gaps: list[TimelineGap] | None = None

    @computed_field
    @property
    def years_covered(self) -> int:
        """Total number of years covered by the timeline."""
        if not self.date_range:
            return 0
        return (self.date_range.end.year - self.date_range.start.year) + 1

    @computed_field
    @property
    def platform_counts(self) -> dict[SourcePlatform, int]:
        """Total memory count per platform."""
        return {p.platform: p.total_memories for p in self.platform_breakdown}

    @computed_field
    @property
    def media_type_counts(self) -> dict[str, int]:
        """Total memory count per media type."""
        return self.media_type_breakdown
...
class Timeline:
    ...
    Example:
        ...
        >>> # Analysis
        >>> stats = timeline.compute_statistics()
        >>> timeline_gaps = timeline.detect_gaps(min_gap_days=30)
        >>>
        >>> # AI preparation
    ...
    def memories(self) -> list[Memory]:
        """All memories in the timeline, in no particular order."""
        return list(self._memories.values())

    @property
    def start_date(self) -> date | None:
        """Earliest memory date in the timeline."""
        if not self._by_date:
            return None
        return min(self._by_date.keys())

    @property
    def end_date(self) -> date | None:
        """Latest memory date in the timeline."""
        if not self._by_date:
            return None
        return max(self._by_date.keys())
    ...
    def get_memories_near(
        self, target_date: date, limit: int = 5, window_days: int = 30
    ) -> list[Memory]:
        """Get memories close to a specific date (within a window).

        Args:
            target_date: Central date to look around.
            limit: Maximum memories to return.
            window_days: Days lookahead/lookbehind.

        Returns:
            List of memories found near the date, chronologically sorted.
        """
        start = target_date - timedelta(days=window_days)
        end = target_date + timedelta(days=window_days)
        memories = self.get_memories_in_range(start, end)

        # If we have too many, pick those closest to target_date
        if len(memories) > limit:
            memories.sort(key=lambda m: abs((m.created_at.date() - target_date).days))
    ...
    def compute_statistics(self) -> TimelineStatistics:
        ...
        if self._by_date:
            ...
            # Find actual datetime bounds for fallback report
            dated_mems = [m for m in self._memories.values() if m.created_at]
            earliest_memory = min(m.created_at for m in dated_mems) if dated_mems else None
            latest_memory = max(m.created_at for m in dated_mems) if dated_mems else None
        ...
        # Detect gaps
        gaps = self.detect_gaps(min_gap_days=30)
        ...
        stats = TimelineStatistics(
            ...
            earliest_memory=earliest_memory if self._by_date else None,
            latest_memory=latest_memory if self._by_date else None,
    ...
    def detect_gaps(self, min_gap_days: int = 30) -> list[TimelineGap]:
        """Find gaps larger than threshold.

        Args:
            min_gap_days: Minimum gap size to detect
    ...
    def get_chapter_candidates(self) -> list[DateRange]:
        ...
        # Get gaps
        gaps = self.detect_gaps(min_gap_days=60)
