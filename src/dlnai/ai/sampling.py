...
from dlnai.config import DepthModeConfig, STANDARD_MODE_CONFIG, DEPTH_MODE_CONFIGS
...
class SamplingStrategy:
    ...
    def detect_duplicates(self, memories: List[Memory]) -> Set[str]:
        ...
        for m in memories:
            ...
            # Heuristic match: same second and similar dimensions
            if m.created_at:
                stamp_key = (m.created_at.replace(microsecond=0), m.width, m.height)
...
def plan_visual_sampling(
    ...
    logger = logging.getLogger("dlnai.ai.sampling")
