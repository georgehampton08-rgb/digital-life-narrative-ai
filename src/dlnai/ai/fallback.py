...
Example:
    >>> from dlnai.ai.fallback import create_fallback_report, is_fallback_mode
    >>>
    >>> if is_fallback_mode():
...
from dlnai.core.timeline import DateRange, Timeline
...
class FallbackAnalyzer:
    ...
    def _create_year_chapters(self, timeline: Timeline) -> list[LifeChapter]:
        ...
        for year in sorted(by_year.keys()):
            ...
            chapter = LifeChapter(
                ...
                media_count=count,
                confidence="low",
                representative_images=memory_ids,
            )
    ...
    def _detect_gaps_simple(self, timeline: Timeline) -> list[DataGap]:
        ...
        timeline_gaps = timeline.detect_gaps(min_gap_days=self._config.min_gap_days)
    ...
    def _create_empty_report(self) -> LifeStoryReport:
        ...
        return LifeStoryReport(
            ...
            executive_summary=(
                "📊 **NO MEMORIES FOUND**\n\n"
                "The analysis could not be performed because no valid memories "
                "were found in the specified source directories."
            ),
            chapters=[],
            is_fallback=True,
...
def is_fallback_mode() -> bool:
    ...
    """
    try:
        from dlnai.ai.client import GENAI_AVAILABLE, get_client
