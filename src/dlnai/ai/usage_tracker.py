...
Example:
    >>> from dlnai.ai.usage_tracker import get_tracker, track_usage
    >>>
...
class UsageTracker:
    ...
    def check_quota(self) -> QuotaStatus:
        ...
        if self._quota_config.daily_request_limit:
            ...
            if usage_pct >= 100:
                ...
                warnings.append(f"Approaching daily request limit ({usage_pct:.0f}% used)")
    ...
    def load(self) -> None:
        ...
        try:
            ...
            with self._lock:
                self._records = [UsageRecord.from_dict(r) for r in records_data]
                self._dirty = False
