...
class ParseResult:
    ...
    def items(self) -> list[Memory]:
        """Alias for memories used in legacy tests."""
        return self.memories

    @property
    def duration_seconds(self) -> float:
        """Alias for parse_duration_seconds used in legacy tests."""
        return self.parse_duration_seconds

    @property
    def source_paths(self) -> list[Path]:
        """Get unique source paths from memories."""
        return list(set(m.source_path for m in self.memories if m.source_path))

    @property
    def stats(self) -> dict[str, Any]:
        """Get parsing statistics."""
        return {
            "items_count": len(self.memories),
            "duration": self.parse_duration_seconds,
