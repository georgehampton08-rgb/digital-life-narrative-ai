...
from dlnai.core.memory import ConfidenceLevel, Memory, SourcePlatform
...
from dlnai.parsers.base import BaseParser, ParseResult, ParseStatus, ProgressCallback
...
def run_pipeline(
    ...
    # Stage 1: Validation & ZIP Extraction
    ...
    for path in paths:
        ...
        if path.is_file() and path.suffix.lower() == ".zip":
            try:
                temp_root = Path(tempfile.mkdtemp(prefix="organizer_zip_"))
                ...
                with zipfile.ZipFile(path, 'r') as zip_ref:
                    ...
                    zip_ref.extractall(temp_root, members=members)
                
                valid_paths.append(temp_root)
            except Exception as e:
                logger.error(f"Failed to extract ZIP {path}: {e}")
                result.total_errors += 1
    ...
    from dlnai.parsers import ParserRegistry
    ...
    if config.parallel_parsing and len(unique_tasks) > 1:
        # Parallel execution
        logger.info(
            f"Parsing {len(unique_tasks)} sources in parallel (max_workers={config.max_workers})"
        )
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if parse_result.status == ParseStatus.FAILED and config.fail_fast:
                        raise PipelineError(
                            f"Parser failed for {parser.platform.value}", "parsing", result
                        )
    ...
    if config.deduplicate_results and all_memories:
        ...
        deduplicated, removed_count = deduplicate_memories(
            all_memories, config.dedup_tolerance_seconds
        )
        result.memories = deduplicated
        result.duplicates_removed = removed_count
        logger.info(
            f"Removed {removed_count} duplicates, {len(deduplicated)} unique memories remain"
        )
    else:
    ...
    result.memories.sort(key=lambda m: (m.created_at is None, m.created_at or datetime.min.replace(tzinfo=timezone.utc)))
...
def parse_single_source(
    path: Path, platform: SourcePlatform, progress: Optional[ProgressCallback] = None
) -> ParseResult:
    ...
    if not parser:
        raise PipelineError(f"No parser available for {platform.value}", "parser_selection")
...
def deduplicate_memories(
    memories: List[Memory], tolerance_seconds: int = 60
    ...
    # Sort by timestamp
    sorted_memories = sorted(memories, key=lambda m: (m.created_at is None, m.created_at or 0))
...
def _merge_duplicate_memories(duplicates: List[Memory]) -> Memory:
    ...
    for dup in sorted_dups:
        ...
        if dup.location and (
            not base.location or dup.location.confidence.value > base.location.confidence.value
        ):
    ...
    # Update merged data
    from dlnai.core.memory import PersonTag
    ...
    for group in time_groups.values():
        ...
        for mem in group:
            if mem.location and mem.location.geo_point:
                if (
                    not best_location
                    or mem.location.confidence.value > best_location.confidence.value
                ):
        ...
        if best_location:
            for mem in group:
                if not mem.location:
                    from dlnai.core.memory import Location
...
def quick_parse(path: Path) -> List[Memory]:
    ...
    if result.has_critical_failures():
        raise PipelineError("Parsing failed", "parsing", result)

    return result.memories


def parse_to_timeline(paths: List[Path], config: Optional[PipelineConfig] = None):
