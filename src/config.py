...
class PrivacyConfig(BaseModel):
    ...
    mode: PrivacyMode = Field(
        default=PrivacyMode.STRICT,
        description="Privacy tier. Higher tiers allow more data."
    )
    send_timestamps: bool = Field(
        default=True,
        description="Allow sending timestamps (essential for timeline analysis)."
    )
    send_locations: bool = Field(
        default=False,
        description="Allow sending location data. Constrained by privacy mode."
    )
    location_precision: Literal["country", "region", "city", "exact"] = Field(
        default="country",
        description="Maximum precision for location data."
    )
    send_captions: bool = Field(
        default=False,
        description="Allow sending caption/description text."
    )
    max_caption_length: int = Field(
        default=100,
        ge=0,
        le=1000,
        description="Maximum caption characters sent to AI."
    )
    send_people_names: bool = Field(
        default=False,
        description="Allow sending people's names."
    )
    hash_people_names: bool = Field(
        default=True,
        description="If sending names, hash them for anonymization."
    )
    send_media_types: bool = Field(
        default=True,
        description="Allow sending media type information."
    )
    send_platform_info: bool = Field(
        default=True,
        description="Allow sending source platform information."
    )
    require_consent_per_session: bool = Field(
        default=True,
        description="Re-prompt for consent each session."
    )
    audit_transmissions: bool = Field(
        default=True,
        description="Log all data transmissions for user review."
    )
...
class PathsConfig(BaseModel):
    ...
    config_dir: Path = Field(
        default_factory=lambda: Path.home() / ".dlna",
        description="Base configuration directory."
    )
    cache_dir: Path | None = Field(
        default=None,
        description="Cache directory. Defaults to config_dir/cache."
    )
    ...
    log_dir: Path | None = Field(
        default=None,
        description="Log directory. Defaults to config_dir/logs."
    )
    encrypted_key_file: Path | None = Field(
        default=None,
        description="Path to encrypted API key file."
    )
    ...
    def resolve_defaults(self) -> "PathsConfig":
        ...
        if self.encrypted_key_file is None:
            ...
            object.__setattr__(
                ...
                Path(self.encrypted_key_file).expanduser().resolve()
            )
...
class ReportConfig(BaseModel):
    ...
    include_statistics: bool = Field(
        default=True,
        description="Include statistical summaries in report."
    )
    include_timeline_visualization: bool = Field(
        default=True,
        description="Include interactive timeline visualization."
    )
    include_raw_ai_responses: bool = Field(
        default=False,
        description="Include raw AI responses (debug feature)."
    )
    theme: Literal["light", "dark", "auto"] = Field(
        default="auto",
        description="Visual theme for HTML reports."
    )
            ...
            try:
                ...
                if content.strip():  # Not empty
                    ...
                    if isinstance(loaded, dict):
                        ...
                        logger.warning(
                            f"Config file {config_file} has unexpected format. "
                            "Using defaults."
                        )
        ...
        if isinstance(ai_data, dict):
            ...
            if "mode" in ai_data and isinstance(ai_data["mode"], str):
                try:
                    ...
                    logger.warning(
                        f"Invalid ai.mode value: {ai_data['mode']}. Using default."
                    )
                    del ai_data["mode"]
...
Examples:
  ...
  python -m src.config --delete-key keyring
    ...
    parser.add_argument(
        "--show", 
        action="store_true", 
        help="Show current configuration (excluding secrets)"
    )
    parser.add_argument(
        "--check-key", 
        action="store_true", 
        help="Check if API key is configured and valid"
    )
    parser.add_argument(
        "--store-key",
        choices=["keyring", "file"],
        help="Store API key (will prompt for key)"
    )
    parser.add_argument(
        "--delete-key",
        choices=["keyring", "file"],
        help="Delete API key from specified source"
    )
