...
class SnapchatParser(BaseParser):
    ...
    supported_extensions: ClassVar[set[str]] = {".json", ".jpg", ".jpeg", ".png", ".mp4", ".mov"}
                    ...
                    for loc_time, geopoint in location_lookup.items():
                        ...
                        if time_diff <= 3600:  # Within 1 hour
                            memory.location = Location(
                                geo_point=geopoint, confidence=ConfidenceLevel.MEDIUM
                            )
                            enriched_count += 1
                            break
    ...
    def _parse_location_history(self, json_path: Path) -> Dict[datetime, GeoPoint]:
        ...
        for entry in location_history:
            try:
                ...
                geopoint = GeoPoint(latitude=latitude, longitude=longitude, altitude=altitude)
    ...
    def _match_memory_to_file(
        self, memory_entry: Dict[str, Any], memories_dir: Path
    ) -> Optional[Path]:
        ...
        if date_str:
            ...
            if memory_time:
                ...
                extensions = (
                    [".jpg", ".jpeg", ".png"] if media_type == "image" else [".mp4", ".mov"]
                )
    ...
    def _parse_snapchat_location(self, entry: Dict[str, Any]) -> Optional[Location]:
        ...
        if lat_str and lon_str:
            try:
                ...
                return Location(
                    geo_point=geopoint, place_name=place_name, confidence=ConfidenceLevel.HIGH
                )
        ...
        if location_str and location_str.strip():
            return Location(place_name=location_str, confidence=ConfidenceLevel.MEDIUM)
