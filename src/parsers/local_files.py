...
FILENAME_PATTERNS = [
    ...
    (r'Screenshot[_ ](\d{4})-(\d{2})-(\d{2})[_ ]at[_ ](\d{1,2})\.(\d{2})\.(\d{2})', 'YYYY-MM-DD_HH.MM.SS'),
...
class LocalFilesParser(BaseParser):
    ...
    def _extract_gps_from_exif(self, exif: Dict[str, Any]) -> Optional[GeoPoint]:
    ...
    def _extract_camera_info(self, exif: Dict[str, Any]) -> Dict[str, Optional[str]]:
    ...
    def _parse_datetime_from_filename(
        ...
        """
        for pattern, format_desc in FILENAME_PATTERNS:
    ...
    def _parse_datetime_from_path(self, path: Path) -> Tuple[Optional[datetime], ConfidenceLevel]:
    ...
    def _infer_context_from_path(self, path: Path) -> Dict[str, Any]:
        ...
        generic_folders = {'photos', 'pictures', 'images', 'videos', 'media', 'camera roll', 'downloads'}
    ...
    def _get_file_system_datetime(self, path: Path) -> Optional[datetime]:
    ...
    def _detect_screenshot(self, filename: str, exif: Dict[str, Any]) -> bool:
        ...
        if ('ios' in software or 'android' in software) and not make:
            return True
        
        return False
    
    def _create_memory_from_file(self, file_path: Path) -> Optional[Memory]:
