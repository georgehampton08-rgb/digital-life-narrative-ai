...
class TestLifeStoryAnalyzer:
    ...
    def test_analyzer_calls_ai_for_chapters(
        ...
        any_called = (
            mock_ai_client.generate_json.called or 
            mock_ai_client.generate_structured.called or
            mock_ai_client.generate.called or
            len(mock_ai_client.method_calls) > 0
        )
    ...
    def test_analyzer_handles_ai_error_gracefully(
        self, sample_memories: list[Memory]
    ) -> None:
        ...
        mock_client.generate_json.return_value = StructuredAIResponse(
            data={"chapters": [{"title": "Test", "start_date": "2020-01-01", "end_date": "2020-12-31"}]},
            raw_text="{}",
            model="gemini-1.5-flash",
            tokens_used=100,
            parse_success=True
...
class TestFallbackAnalyzer:
    ...
    def test_fallback_analyze_returns_report(
        self, sample_memories: list[Memory]
    ) -> None:
    ...
    def test_fallback_report_is_fallback_mode(
        self, sample_memories: list[Memory]
    ) -> None:
    ...
    def test_fallback_ai_model_indicates_none(
        self, sample_memories: list[Memory]
    ) -> None:
    ...
    def test_fallback_chapters_by_year(
        self, sample_memories: list[Memory]
    ) -> None:
    ...
    def test_fallback_narratives_indicate_unavailable(
        self, sample_memories: list[Memory]
    ) -> None:
        ...
        for chapter in result.chapters:
            ...
            assert (
                "unavailable" in narrative_lower or 
                "ai" in narrative_lower or 
                "fallback" in narrative_lower or
                "statistics" in narrative_lower or
                "configure" in narrative_lower
            )

    def test_fallback_stats_accurate(
        self, sample_memories: list[Memory]
    ) -> None:
...
class TestContentFilter:
    ...
    def test_filter_returns_result(
        ...
        with patch("src.ai.content_filter.check_and_prompt_disclosure", return_value=sample_safety_settings):
    ...
    def test_filter_flags_sensitive_captions(
        self, sample_safety_settings: SafetySettings
    ) -> None:
    ...
    def test_filter_respects_safety_action(
        self, sample_safety_settings: SafetySettings
    ) -> None:
    ...
    def test_filter_pixel_analysis_requires_consent(
        self, sample_memories: list[Memory]
    ) -> None:
        """Pixel analysis without consent skips AI analysis."""
        settings = SafetySettings(
            use_pixel_analysis=True,
            pixel_analysis_disclosure_acknowledged=False
        )
...
class TestSafetyHelpers:
    ...
    def test_is_visually_safe_allow(self) -> None:
        ...
        state = MemorySafetyState(
            memory_id="test-id",
            resolved_action=SafetyAction.ALLOW
        )
    ...
    def test_is_visually_safe_hide(self) -> None:
        ...
        state = MemorySafetyState(
            memory_id="test-id",
            resolved_action=SafetyAction.HIDE_FROM_REPORT
        )
...
class TestAIFallbackIntegration:
    ...
    def test_partial_ai_failure_still_produces_report(
        self, sample_memories: list[Memory]
    ) -> None:
