...
class TestDetection:
    ...
    def test_detect_google_photos_export(self, google_photos_export_dir: Path) -> None:
        ...
        assert len(results) >= 1
        google_result = next((r for r in results if r.platform == SourcePlatform.GOOGLE_PHOTOS), None)
...
class TestParserRobustness:
    ...
    def test_parser_handles_corrupt_json(self, tmp_path: Path) -> None:
        ...
        assert result.status in [ParseStatus.PARTIAL, ParseStatus.FAILED] or len(result.warnings) > 0 or len(result.errors) > 0
...
@pytest.mark.parametrize("platform,parser_class", [
